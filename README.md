...
**A child still running ends the attempt, not the job.** A parent can reach its dispatch while one of its children is still in flight — a child that was retried after the parent was already queued behind it. The runtime counts the children that have not settled and fails that attempt before the handler, retryably. The message names the job and how many are still running. The attempt after it reads a full set and runs, so nothing is lost and no handler ran over a slot that was still filling. It is an ordinary failed attempt and it spends one, so a parent that keeps meeting an unsettled child ends like any other job that runs out: `attempts_exhausted`.
